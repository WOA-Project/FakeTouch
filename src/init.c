...
--*/
{
	FKE_CONTROLLER_CONTEXT* controller;
	...
	controller = (FKE_CONTROLLER_CONTEXT*)ControllerContext;
	interruptStatus = 0;
	status = STATUS_SUCCESS;

	//
	// Populate context with FKE function descriptors
	//
	status = FkeBuildFunctionsTable(
		ControllerContext,
		SpbContext);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_INIT,
			"Could not build table of FKE functions - 0x%08lX",
			status);
		goto exit;
	}

	//
	// Initialize FKE function control registers
	//
	status = FkeConfigureFunctions(
		ControllerContext,
		SpbContext);
	...
	status = FkeConfigureInterruptEnable(
		ControllerContext,
		SpbContext);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_INIT,
			"Could not get FKE firmware version - 0x%08lX",
			status);
		goto exit;
	}
...
--*/
{
	FKE_CONTROLLER_CONTEXT* context;
	NTSTATUS status;
	...
	RtlZeroMemory(context, sizeof(FKE_CONTROLLER_CONTEXT));
