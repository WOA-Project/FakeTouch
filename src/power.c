/*++
	...
	Abstract:
		...
		Contains Fake power-on and power-off functionality
...
)
{
	...
	FKE_CONTROLLER_CONTEXT* ControllerContext = NULL;
	...
	ControllerContext = (FKE_CONTROLLER_CONTEXT*)devContext->TouchContext;
	...
	if (IsEqualGUID(&GUID_ACDC_POWER_SOURCE, SettingGuid))
	{
		...
		case PoAc:
			...
			status = FkeChangeChargerConnectedState(
				ControllerContext,
				SpbContext,
				0
			);
